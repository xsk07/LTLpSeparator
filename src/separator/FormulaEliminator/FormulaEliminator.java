...
public abstract class FormulaEliminator {
    ...
    public static Formula applyElimination(JunctionPath lj, JunctionPath rj) {

        BinaryFormula x = null;
        if(lj != null) x = lj.getX();
        else if(rj != null) x = rj.getX();

        if(lj != null && rj != null) {
            BinaryFormula lx = lj.getX();
            BinaryFormula rx = rj.getX();
            if(!(lx == rx)) {
    ...
    public static int eliminationNumber(JunctionPath lj, JunctionPath rj) {
        if(junctionCase(lj) == 0) { /* if there is no left junction */
            ...
            if(junctionCase(lj) == 1) {
                ...
                if(junctionCase(rj) == 1) {
                    BinaryFormula ly = lj.getY();
                    BinaryFormula ry = rj.getY();
     ...
     * [0] == a, [1] == A, [2] == B, [3] == q */
    protected static Formula[] getSubformulas(JunctionPath lj, JunctionPath rj, int c){
        return switch(c) {
    ...
    protected static Formula[] subformulas1(JunctionPath lj){
        BinaryFormula x = lj.getX();
        BinaryFormula y = lj.getY();
    ...
    /** S(a, q | U(A,B)) */
    // pre: f.getOperator() == SINCE/UNTIL
    protected static Formula[] subformulas2(JunctionPath rj){
        BinaryFormula x = rj.getX();
        BinaryFormula y = rj.getY();
    ...
    /** S(a & !U(A,B), q) */
    // pre: f.getOperator() == SINCE/UNTIL
    protected static Formula[] subformulas3(JunctionPath lj){
        BinaryFormula x = lj.getX();
        BinaryFormula y = lj.getY();
        BinaryFormula lAnd = (BinaryFormula) x.getLoperand();
        ArrayList<Formula> as = lAnd.getCombinationOperands();
    ...
    /** S(a, q | !U(A,B)) */
    // pre: f.getOperator() == SINCE/UNTIL
    protected static Formula[]  subformulas4(JunctionPath rj){
        BinaryFormula x = rj.getX();
        BinaryFormula y = rj.getY();
        BinaryFormula rOr = (BinaryFormula) x.getRoperand();
        ArrayList<Formula> qs = rOr.getCombinationOperands();
    ...
    /** S(a & U(A,B), q | U(A,B)) */
    // pre: f.getOperator() == SINCE/UNTIL
    protected static Formula[] subformulas5(JunctionPath lj, JunctionPath rj){
        BinaryFormula x = lj.getX();
    ...
    /** S(a & !U(A,B), q | U(A,B)) */
    // pre: f.getOperator() == SINCE/UNTIL
    protected static Formula[] subformulas6(JunctionPath lj, JunctionPath rj) {
        BinaryFormula x = lj.getX();
    ...
    /** S(a & U(A,B), q | !U(A,B)) */
    // pre: f.getOperator() == SINCE/UNTIL
    protected static Formula[] subformulas7(JunctionPath lj, JunctionPath rj) {
        BinaryFormula x = lj.getX();
    ...
    /** S(a & !U(A,B), q | !U(A,B)) */
    // pre: f.getOperator() == SINCE/UNTIL
    protected static Formula[] subformulas8(JunctionPath lj, JunctionPath rj) {
        BinaryFormula x = lj.getX();
